...
truePositive = apply(decisions,1, \(x) sum(x * trialData$effect)),
falsePositive = apply(decisions,1, \(x) sum(x * (1-trialData$effect))))

## add Bonferroni with beta = NA

bonf_decision = as.numeric(p_vals <= alpha/K)
results = results |> bind_rows(
tibble(sim =r, beta = NA, truePositive = sum(bonf_decision * trialData$effect), falsePositive =  sum(bonf_decision * (1-trialData$effect)))
)
