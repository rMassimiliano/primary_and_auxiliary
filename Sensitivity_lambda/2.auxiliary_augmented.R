...
setwd("../primary_and_auxiliary/multiple_tests_K2/")
data_path = 'data/'
...
{
  R = 5000
  dec = array(0,dim =c(R, 2,NROW(optimal_solutions)))
  for(r in 1:R)
  {
  ...
  for(b in 1:NROW(optimal_solutions))
  {
  BETA = optimal_solutions$beta[b]
  w = distribute_weights(BETA,mean_S)
  p_vals = 1-pnorm(mean_Y/sqrt(var_Y))
  dec[r,,b] = as.numeric(p_vals <= w*ALPHA)
  }
  }
A_auxiliary_results[[current_scenario]] = dec
...
setwd(cpath)
save(A_auxiliary_results, file ="A_auxiliary_results_sensitivity.rdata")
