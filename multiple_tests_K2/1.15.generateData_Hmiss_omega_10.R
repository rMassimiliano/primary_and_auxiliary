...
true_parameters$p_k = c(0.6,0.4)        ## biomarker proportion

## marginals for the primary
true_parameters$pi_ykc = array(0, dim = c(true_parameters$K, 2))
## SOC
true_parameters$pi_ykc[,1] = c(0.20,0.20)
## no TE
##true_parameters$pi_ykc[,2] = c(0.20,0.20) 
true_parameters$pi_ykc[,2] = c(0.40,0.20) 
...
## auxiliary SOC
true_parameters$pi_skc[,1] = c(0.75,0.50)
## auxiliary TE
 # no effect on group 4
 # 
true_parameters$pi_skc[,2] = c(0.50, 0.50)


## odds ration copula parameters
## 1: independent
true_parameters$rho = 10
...
readr::write_csv(trialData, sprintf("data/Hmiss_omega_10/trial_Hmiss_omega_%i_%i.csv",true_parameters$rho, r))
