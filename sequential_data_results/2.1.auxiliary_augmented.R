...
scenario_list = c('H01_omega_1',    'H0_omega_1',     'H10_omega_1',    'H11_omega_1', 'Hmiss_omega_1', 'H01_omega_10', 'H0_omega_10', 'H10_omega_10', 'H11_omega_10', 'Hmiss_omega_10', 'H01_omega_2', 'H0_omega_2', 'H10_omega_2', 'H11_omega_2', 'Hmiss_omega_2')
  ...
  Zy_IA = with(dat[1:100,], (mean(Y[C_i ==1]) - mean(Y[C_i ==0]))/sqrt(var(Y[C_i ==1])/sum(C_i==1) + var(Y[C_i ==0])/sum(C_i==0)))
  Zy_FA = with(dat, (mean(Y[C_i ==1]) - mean(Y[C_i ==0]))/sqrt(var(Y[C_i ==1])/sum(C_i==1) + var(Y[C_i ==0])/sum(C_i==0)))
  
  stan_input = list( y_interim = dat$Y[1:100],
                     s_interim = dat$S[1:150],
                     c = dat$C_i,
                     n =NROW(dat),
                     ny_interim =100,
                     ny_future = 100,
...
data_path = 'data/'
model = readRDS("clean_code_stan/joint_model_interim.rds")
...
saveRDS(decision, file =  paste0("results/auxiliary_augmented/","auxiliary_augmented_",current_scenario,".rds") )
