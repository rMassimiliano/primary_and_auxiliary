...
## there is an effect for both primary and auxiliary---we hope that most of the time we go on with the trial and reject the null
...
for(current_scenario in scenarios_names)
{
 ...
 for(r in 1:length(data_list))
 {
  dat = read.csv(sprintf('%s%s/%s',data_path,current_scenario,data_list[r]))
    ...
    if(C_power<= 0.8)  dec  = 'futility-stop'
 ...
 freq_analysis[[current_scenario]] = decision
}

save(freq_analysis, file = 'freq_analysis.rdata')
