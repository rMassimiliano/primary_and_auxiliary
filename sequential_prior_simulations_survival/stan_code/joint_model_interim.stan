...
int<lower = 0, upper =1> treat_IA_censored[n_IA_pfs_censored]; // treatment allocation (0 - control & 1 - treated)
int<lower = 0, upper =1> treat_IA_uncensored[n_IA_pfs_uncensored]; // treatment allocation (0 - control & 1 - treated)
...
{
 target += exponential_lccdf(pfs_interim_censored[ic] | exp(-intercept_pfs - slope_pfs*treat_IA_censored[ic]));
}

// likelihood PFS uncensored
for(iu in 1:n_IA_pfs_uncensored)
{
 pfs_interim_uncensored[iu] ~ exponential(exp(-intercept_pfs - slope_pfs*treat_IA_uncensored[iu]));
}

// likelihood SPP censored
for(jc in 1:n_IA_spp_censored)
{
 target += exponential_lccdf(spp_interim_censored[jc] | exp(-intercept_spp ));
}

// likelihood SPP uncensored
for(ju in 1:n_IA_spp_uncensored)
{
spp_interim_uncensored[ju] ~ exponential(exp(-intercept_spp ));
}
...
real<lower=0> spp_future[n_future_spp]; 
 ...
 for(i in 1:n_future_pfs)
 {
  ...
  {
   spp_future[j] = exponential_rng(exp(-intercept_spp));
  }
}
